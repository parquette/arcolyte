...
pub unsafe extern "C" fn request_send(req: *const Request) -> *mut Response {
    if req.is_null() {
        return error_ptr(Error::from("Received null pointer"));
    }
    ...
    let response = match send_request(req) {
        Ok(r) => r,
        Err(e) => {
            return error_ptr(Error::with_chain(e, "Sending request failed."));
        }
    };
...
#[no_mangle]
pub unsafe extern "C" fn plugin_manager_new() -> *mut PluginManager {
